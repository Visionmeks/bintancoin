...
const char * const BITCOIN_CONF_FILENAME = "bintancoin.conf";
const char * const BITCOIN_PID_FILENAME = "bintancoind.pid";
...
#else
    const char* pszModule = "bintancoin";
#endif
    if (pex)
        return strprintf(
...
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "bintancoin";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/bintancoin";
#else
    // Unix
    return pathRet / ".bintancoin";
#endif
#endif
}
